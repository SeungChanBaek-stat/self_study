...
이러한 고차원 시뮬레이션 데이터셋에 대한 능형회귀 실험결과, MSE기준에서 능형회귀가 최소제곱회귀보다 좋다는 점을 알 수 있다. 그러나 MSE기준이 아닌 다른 기준 (supreme norm, euclidean norm 등)에서는 최소제곱회귀보다 좋은 점을 찾기 어려움도 알 수 있다. 

<!-- 그리고 $MSE(\hat{\beta}(k))$가 $k$에 대한 함수이면서 $k \rightarrow 0$일 때 최소제곱회귀추정량에 가까워진다는 점과 $k \rightarrow \infty$일 때 0벡터에 가까워진다는 점도 확인할 수 -->